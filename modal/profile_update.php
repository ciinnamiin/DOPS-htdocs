...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["user_id"])) {
    ...
    if(isset($_POST["password"]) && $_POST["password"] != '' ){
        ...
        if($_POST["password"] === $_POST["confirm_password"]){
            ...
            $p .= ", `password` = '$password', pwd_updated = 1 ";
        }else{
            $valid = 0;
    ...
    if($valid == 1){
        ...
        if(verifyPassword($current_password, $employee_id)){  
...
function verifyPassword($password, $employee_id){
    global $conn;

    $sql = "SELECT * FROM employees WHERE employee_id LIKE '$employee_id' "; // Adjust the SQL query as needed
    $result = $conn->query($sql);
